...
These rules apply to ALL GitLab operations.

## Confirmations

Critical GitLab actions (`issue create`, `mr create`, `mr update`, `push`) require developer confirmation unless `-y` was passed.

## Restrictions

- Never merge MRs without permission
- Never dismiss reviews without permission
- Never add promotional text like "Generated with Claude Code" to MR descriptions
- When posting MR comment replies, include the commit SHA and description of what was fixed
...
Determine emoji from the **GitLab issue** title and labels:
